...
#'
#' A package which uses exponential shifting and Fast Fourier Transformations with the FFTW library to
...
dpoisbin <-
  function(x,
    ...
    if (method == "ShiftConvolve") {
      return(shift_pb_density(
        x = x,
        probs = probs,
        log.p = log.p
      ))
    } else if (method == "DC") {
      return(dc_pb_density(
        x = x,
        probs = probs,
        log.p = log.p
      ))
  ...
  function(p,
    ...
    if (lower.tail) {
      for (prob in sort(unique(p))) {
        ...
        while (curr_cdf_pos < cdf_length &&
          cdf[curr_cdf_pos] <= prob) {
...
process <- function(p) {
  ...
  n1 <- sum(p == 1)
  if ((n0 != 0) | (n1 != 0)) {
    # print(paste("Input vector contains", n0, "0s and", n1, "1s"))
    p <- p[(p != 0) & (p != 1)]
  }
  return(c(n0, n1, p))
}
  ...
  if (S0 == -1) {
    ...
    U <-
      uniroot(shiftedmean,
        ...
        S0,
        tol = 10^-16
      ) # finds a suitable shifting parameter
    t0 <- U$root
  }
    ...
    if (y == n - 1) {
      ...
      U <-
        uniroot(shiftedmean,
          ...
          y - 1,
          tol = 10^-16
        ) # find a suitable shifting parameter
      t0 <- U$root
    } else {
      if (log.p) {
  ...
  for (i in 1:length(x_ordinary)) {
    if (lower.tail) {
      if (t0 > 0) {
        if (log.p) {
          log_right_tail <-
            logsum(pmf_computed[c((x_ordinary[i] + 2):length(pmf_computed))])
          p <- 1 - exp(log_right_tail)
          lp <- log(p)
          return_dist[x_return_index[i]] <- lp
        } else {
          right_tail <-
            sum(pmf_computed[c((x_ordinary[i] + 2):length(pmf_computed))])
    ...
    if (!is.null(edge)) {
      ...
      if (log.p) {
        if (lower.tail) {
          ...
          return_dist[i] <-
            logsum(pmf_computed[c((x[i] + 2):length(pmf_computed))])
        }
      } else {
        if (lower.tail) {
          ...
          return_dist[i] <-
            sum(pmf_computed[c((x[i] + 2):length(pmf_computed))])
        }
      }
    }
  }
...
dc_pb_density <- function(x, probs, log.p = FALSE) {
  ...
  for (i in 1:length(x)) {
    ...
    if (!is.null(edge)) {
      ...
      return_density[i] <-
        pmf_computed[x[i] + 1] # Previous if block would have already decided log.p or not
    }
  }
  return(return_density)
}
